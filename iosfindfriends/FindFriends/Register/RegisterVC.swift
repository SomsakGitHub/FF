...
class RegisterVC: BaseViewController, RegisterModelDelegate {
    ...
    func didFinishAuthentication(_ status: statusWebService, code: String) {
        print("code", code)
        self.stopAnimating()
    ...
    func didFinishRegister(_ status: statusWebService) {
        self.stopAnimating()
        switch status {
        case .success:
